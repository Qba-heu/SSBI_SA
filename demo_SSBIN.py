...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(self, global_pool=False, **kwargs):
        ...
        # self.multiorder_agg = MultiOrderGatedAggregation(embed_dim ,attn_dw_dilation=[0,0,0])
        self.spec_agg = attn_block(dim=embed_dim,num_heads=kwargs['num_heads'])
        self.agg_norm = norm_layer(embed_dim)
        self.lambda1 = 0.9
        # self.att_blocks = nn.ModuleList([
        #     attn_block(
        #         dim=embed_dim, num_heads=kwargs['num_heads'])
...
model = vit_base_patch16(num_classes=9,
            drop_path_rate=0.1,
            global_pool=True,
            img_size = 13,
            patch_size = 2,
...
input = torch.rand((2,103,13,13)).cuda()
model.train(True)
out = model(input)
print(out.shape)